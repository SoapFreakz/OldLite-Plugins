...
export default {
  ...
  init(api) {
    ...
    function ensureGearButton() {
      ...
      gearBtn.id = 'tileman-gear';
      gearBtn.textContent = '\u2699';
      gearBtn.title = 'Tileman settings';
      gearBtn.style.cssText = [
        'position: absolute', 'width: 24px', 'height: 24px', 'line-height: 24px',
        'text-align: center', 'background: rgba(0,0,0,0.75)', 'border: 1px solid #5a4a30',
        'border-radius: 4px', 'color: #ff981f', 'font-size: 15px', 'cursor: pointer',
        'z-index: 10001', 'user-select: none',
      ].join(';');
      gearBtn.addEventListener('click', openSettingsPanel);
      gameCanvas.parentElement.appendChild(gearBtn);
      positionGearButton();
    }

    function positionGearButton() {
      const gameCanvas = api.getGameCanvas();
      if (!gearBtn || !gameCanvas) return;
      gearBtn.style.left = gameCanvas.offsetLeft + gameCanvas.offsetWidth - 24 - 8 + 'px';
      gearBtn.style.top = gameCanvas.offsetTop + 8 + 'px';
    }
    ...
    function ensureSettingsPanel() {
      ...
      const style = document.createElement('style');
      style.textContent = `
        #tileman-settings-panel { width: 340px; max-width: 92vw; background: linear-gradient(180deg, rgba(28,22,14,0.97), rgba(17,13,8,0.97)); border: 1px solid #4a3b24; border-radius: 8px; padding: 18px 20px 16px; font-family: monospace; font-size: 13px; color: #d9b478; box-shadow: 0 12px 32px rgba(0,0,0,0.55); }
        #tileman-settings-panel .tm-settings-header { display: flex; justify-content: space-between; align-items: center; font-size: 16px; font-weight: bold; color: #ffb347; margin-bottom: 14px; padding-bottom: 10px; border-bottom: 1px solid #4a3b24; }
        #tileman-settings-panel .tm-settings-header span:last-child { cursor: pointer; font-size: 19px; color: #8a734c; }
        #tileman-settings-panel .tm-section-label { font-size: 10.5px; letter-spacing: 1.2px; text-transform: uppercase; color: #8a734c; margin: 18px 0 8px; }
        #tileman-settings-panel .tm-section-label:first-of-type { margin-top: 0; }
        #tileman-settings-panel .tm-settings-row { display: grid; grid-template-columns: 1fr auto; align-items: center; gap: 10px; padding: 5px 0; }
        #tileman-settings-panel .tm-settings-row.tm-row-full { grid-template-columns: 1fr; padding-top: 2px; padding-bottom: 10px; }
        #tileman-settings-panel .tm-settings-row label { color: #cba879; }
        #tileman-settings-panel .tm-opacity-val { color: #ffe4b3; font-weight: bold; min-width: 42px; text-align: right; }
        #tileman-settings-panel input[type="number"], #tileman-settings-panel select { width: 100px; box-sizing: border-box; background: #14100a; color: #ffe4b3; border: 1px solid #4a3b24; border-radius: 5px; padding: 6px 8px; font-family: monospace; font-size: 13px; text-align: right; }
        #tileman-settings-panel select { text-align: left; cursor: pointer; }
        #tileman-settings-panel input[type="color"] { width: 100px; height: 32px; padding: 0; border: 1px solid #4a3b24; border-radius: 5px; background: transparent; cursor: pointer; }
        #tileman-settings-panel input[type="range"] { width: 100%; accent-color: #ff981f; }
        #tileman-settings-panel .tm-settings-checkbox-row { grid-template-columns: 1fr; justify-items: start; }
        #tileman-settings-panel .tm-settings-checkbox-row label { display: flex; align-items: center; gap: 8px; color: #cba879; cursor: pointer; }
        #tileman-settings-panel input[type="checkbox"] { width: 15px; height: 15px; accent-color: #ff981f; cursor: pointer; }
        #tileman-settings-panel #tm-restriction-fields { margin: 4px 0 2px 2px; padding: 2px 0 2px 14px; border-left: 2px solid #3a2f1c; }
        #tileman-settings-panel .tm-settings-actions { display: flex; gap: 10px; margin-top: 18px; padding-top: 14px; border-top: 1px solid #4a3b24; }
        #tileman-settings-panel .tm-btn { flex: 1; padding: 9px 8px; font-family: monospace; font-size: 12px; font-weight: bold; border-radius: 6px; border: 1px solid transparent; cursor: pointer; color: #fff; }
        #tileman-settings-panel .tm-btn-danger { background: #5c2323; border-color: #7a3232; }
        #tileman-settings-panel .tm-btn-primary { background: #285129; border-color: #3a6b3d; }
      `;
      api.container.appendChild(style);

      settingsBackdrop = document.createElement('div');
      settingsBackdrop.id = 'tileman-settings-backdrop';
      settingsBackdrop.style.cssText = [
      ...
      settingsBackdrop.innerHTML = `
        <div id="tileman-settings-panel">
          <div class="tm-settings-header"><span>Tileman Settings</span><span id="tm-settings-close">&times;</span></div>
          <div class="tm-section-label">Appearance</div>
          <div class="tm-settings-row"><label>Tile Opacity</label><span id="tm-opacity-val" class="tm-opacity-val">100%</span></div>
          <div class="tm-settings-row tm-row-full"><input type="range" id="tm-opacity" min="0" max="100" /></div>
          <div class="tm-settings-row"><label>Available Color</label><input type="color" id="tm-color-available" /></div>
          <div class="tm-settings-row"><label>Restricted Color</label><input type="color" id="tm-color-restricted" /></div>
          <div class="tm-settings-row"><label>Textbox Scale</label>
            <select id="tm-textbox-scale"><option value="0.25">x0.25</option><option value="0.5">x0.5</option><option value="1">x1</option><option value="2">x2</option></select>
          </div>
          <div class="tm-section-label">Tile Cost</div>
          <div class="tm-settings-row"><label>XP per Tile</label><input type="number" id="tm-xp-per-tile" min="1" /></div>
          <div class="tm-settings-row tm-settings-checkbox-row"><label><input type="checkbox" id="tm-manual-unlocks" /> Manual Unlocks</label></div>
          <div class="tm-settings-row tm-settings-checkbox-row"><label><input type="checkbox" id="tm-restriction-enabled" /> Progressive Cost Increase</label></div>
          <div id="tm-restriction-fields">
            <div class="tm-settings-row"><label>Every (Total XP)</label><input type="number" id="tm-restriction-step-xp" min="1" /></div>
            <div class="tm-settings-row"><label>Increase Cost By</label><input type="number" id="tm-restriction-step-increase" min="1" /></div>
          </div>
          <div class="tm-settings-actions">
            <button id="tm-reset-btn" class="tm-btn tm-btn-danger">Reset All Tiles</button>
            <button id="tm-save-btn" class="tm-btn tm-btn-primary">Save Settings</button>
          </div>
        </div>
      `;
      api.container.appendChild(settingsBackdrop);

      settingsBackdrop.addEventListener('click', (e) => {
        if (e.target === settingsBackdrop) closeSettingsPanel();
      });
      settingsBackdrop.querySelector('#tm-settings-close').addEventListener('click', closeSettingsPanel);

      const opacityInput = settingsBackdrop.querySelector('#tm-opacity');
      const opacityVal = settingsBackdrop.querySelector('#tm-opacity-val');
      const colorAvailableInput = settingsBackdrop.querySelector('#tm-color-available');
      const colorRestrictedInput = settingsBackdrop.querySelector('#tm-color-restricted');
      const textboxScaleInput = settingsBackdrop.querySelector('#tm-textbox-scale');
      const xpPerTileInput = settingsBackdrop.querySelector('#tm-xp-per-tile');
      const manualUnlocksInput = settingsBackdrop.querySelector('#tm-manual-unlocks');
      const restrictionEnabledInput = settingsBackdrop.querySelector('#tm-restriction-enabled');
      const restrictionFieldsEl = settingsBackdrop.querySelector('#tm-restriction-fields');
      const restrictionStepXpInput = settingsBackdrop.querySelector('#tm-restriction-step-xp');
      const restrictionStepIncreaseInput = settingsBackdrop.querySelector('#tm-restriction-step-increase');

      opacityInput.addEventListener('input', () => {
        settingsDraft.opacity = parseInt(opacityInput.value, 10) / 100;
        opacityVal.textContent = opacityInput.value + '%';
      });
      colorAvailableInput.addEventListener('input', () => (settingsDraft.colorAvailable = colorAvailableInput.value));
      colorRestrictedInput.addEventListener('input', () => (settingsDraft.colorRestricted = colorRestrictedInput.value));
      textboxScaleInput.addEventListener('change', () => {
        const v = parseFloat(textboxScaleInput.value);
        settingsDraft.textboxScale = TEXTBOX_SCALE_FRACTIONS.hasOwnProperty(v) ? v : settingsDraft.textboxScale;
      });
      xpPerTileInput.addEventListener('input', () => {
        const v = parseInt(xpPerTileInput.value, 10);
        settingsDraft.xpPerTile = Number.isFinite(v) && v > 0 ? v : settingsDraft.xpPerTile;
      });
      manualUnlocksInput.addEventListener('change', () => (settingsDraft.manualUnlocks = manualUnlocksInput.checked));
      restrictionEnabledInput.addEventListener('change', () => {
        settingsDraft.restrictionEnabled = restrictionEnabledInput.checked;
        restrictionFieldsEl.style.display = settingsDraft.restrictionEnabled ? 'block' : 'none';
      });
      restrictionStepXpInput.addEventListener('input', () => {
        const v = parseInt(restrictionStepXpInput.value, 10);
        settingsDraft.restrictionStepXp = Number.isFinite(v) && v > 0 ? v : settingsDraft.restrictionStepXp;
      });
      restrictionStepIncreaseInput.addEventListener('input', () => {
        const v = parseInt(restrictionStepIncreaseInput.value, 10);
        settingsDraft.restrictionStepIncrease = Number.isFinite(v) && v > 0 ? v : settingsDraft.restrictionStepIncrease;
      });

      settingsBackdrop.querySelector('#tm-reset-btn').addEventListener('click', () => {
        if (confirm("This will permanently reset ALL of your unlocked tiles. This can't be undone. Are you sure?")) {
          resetAllTiles();
        }
      });

      settingsBackdrop.querySelector('#tm-save-btn').addEventListener('click', () => {
        const costChanged =
          settingsDraft.xpPerTile !== settings.xpPerTile ||
          settingsDraft.restrictionEnabled !== settings.restrictionEnabled ||
          settingsDraft.restrictionStepXp !== settings.restrictionStepXp ||
          settingsDraft.restrictionStepIncrease !== settings.restrictionStepIncrease;
        if (costChanged) {
          if (!confirm('Changing how much xp a tile costs will reset all of your currently unlocked tiles, to keep things fair. Continue?')) return;
          resetAllTiles();
        }
    ...
    function populateSettingsPanel() {
      const pct = Math.round(settingsDraft.opacity * 100);
      settingsBackdrop.querySelector('#tm-opacity').value = pct;
      settingsBackdrop.querySelector('#tm-opacity-val').textContent = pct + '%';
      settingsBackdrop.querySelector('#tm-color-available').value = settingsDraft.colorAvailable;
      settingsBackdrop.querySelector('#tm-color-restricted').value = settingsDraft.colorRestricted;
      settingsBackdrop.querySelector('#tm-textbox-scale').value = String(settingsDraft.textboxScale);
      settingsBackdrop.querySelector('#tm-xp-per-tile').value = settingsDraft.xpPerTile;
      settingsBackdrop.querySelector('#tm-manual-unlocks').checked = settingsDraft.manualUnlocks;
      const restrictionEnabledInput = settingsBackdrop.querySelector('#tm-restriction-enabled');
      restrictionEnabledInput.checked = settingsDraft.restrictionEnabled;
      settingsBackdrop.querySelector('#tm-restriction-fields').style.display = settingsDraft.restrictionEnabled ? 'block' : 'none';
      settingsBackdrop.querySelector('#tm-restriction-step-xp').value = settingsDraft.restrictionStepXp;
      settingsBackdrop.querySelector('#tm-restriction-step-increase').value = settingsDraft.restrictionStepIncrease;
    }
  ...
  destroy(api) {
    ...
    // and removes api.container (which the HUD, settings backdrop, and
    // injected <style> tags were all appended into). Two things live
    // OUTSIDE api.container on purpose — the gear button and the
    // login-screen wordmark are position:absolute against the game
    // canvas's own parent (so their offsetLeft/offsetTop math lines up
    // with the canvas), not against api.container — so they need explicit
    // removal here, along with the contextmenu listener on the raw canvas.
    const gearBtn = document.getElementById('tileman-gear');
